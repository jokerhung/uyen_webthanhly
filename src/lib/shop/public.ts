...
export type PublicShop = Pick<ShopSettings, "shopName" | "primaryColor" | "backgroundColor" | "surfaceColor" | "address" | "facebookUrl" | "phone" | "opensAt" | "closesAt">;
export async function getPublicShop(): Promise<PublicShop> {
  const { shopName, primaryColor, backgroundColor, surfaceColor, address, facebookUrl, phone, opensAt, closesAt } = await getShopSettings();
  return { shopName, primaryColor, backgroundColor, surfaceColor, address, facebookUrl, phone, opensAt, closesAt };
}
export const shopOpeningHours = (shop: PublicShop) => `${shop.opensAt}–${shop.closesAt}${shop.closesAt < shop.opensAt ? " (ngày hôm sau)" : ""}`;
export const shopAnnouncement = (shop: PublicShop) => `${shop.shopName} ✦ ${shop.address} ✦ ${shop.phone}`;
...
export const shopTagline = "From one bestie to another ♡";
