...
export type PublicShop = Pick<ShopSettings, "shopName" | "slogan" | "primaryColor" | "backgroundColor" | "surfaceColor" | "address" | "facebookUrl" | "phone" | "opensAt" | "closesAt">;
export async function getPublicShop(): Promise<PublicShop> {
  const { shopName, slogan, primaryColor, backgroundColor, surfaceColor, address, facebookUrl, phone, opensAt, closesAt } = await getShopSettings();
  return { shopName, slogan, primaryColor, backgroundColor, surfaceColor, address, facebookUrl, phone, opensAt, closesAt };
}
