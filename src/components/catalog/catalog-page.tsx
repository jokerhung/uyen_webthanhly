...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <CatalogFilters groups={groups} selected={selected} total={total}>
    {items.length ? <div className="catalog__grid">{items.map(item => <article className="catalog__card" key={item.id}>
      ...
      <div className="catalog__card-info"><p className="catalog__brand">{item.brand?.label ?? "H.U.N"}</p><Link href={`/items/${encodeURIComponent(item.slug)}`} className="catalog__name">{item.name}</Link><p className="catalog__price">{money(item.salePrice)}</p><Link className="catalog__quick-link" href={`/items/${encodeURIComponent(item.slug)}`} aria-label={`Chi tiết ${item.name}`}>+</Link></div>
